...
class AsyncS3Client:
    ...
    async def list_files(self, bucket_name: str, prefix: Optional[str] = "") -> List[str]:
    ...
    async def contains_expected_sensors_file(self, bucket_name: str, prefix: Optional[str] = "") -> bool:
        """
    ...
    async def download_and_parse_expected_sensors_file(
        ...
        """
        files = await self.list_files(bucket_name, prefix)
        target_file = next((key for key in files if constants.EXPECTED_SENSORS_FILENAME in key), None)
        if not target_file:
            return None
